...
class AttentiveLayer(nn.Module):
    ...
    def forward(self, x, x_size, params):
        ...
        b, n, c = x.shape
        c3 = 3 * c
        layer_condition = params.get("layer_condition") if isinstance(params, dict) else None
        attn_condition = None
        ssm_condition = None
        if layer_condition is not None:
            attn_condition, ssm_condition = layer_condition.chunk(2, dim=-1)
        ...
        x_win = self._apply_condition(x_win, attn_condition)
        ...
        x = self._apply_condition(x, ssm_condition)
...
class MambaIRv2(nn.Module):
    ...
    def forward_features(self, x, params, cond_embedding=None):
...
class DegradationAwareMambaIRv2(MambaIRv2):
    def __init__(
        self,
        *args,
        degradation_dim=64,
        degradation_hidden_dim=64,
        degradation_condition_scale=0.1,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.degradation_dim = degradation_dim
        self.degradation_condition_scale = degradation_condition_scale
        self.degradation_encoder = nn.Sequential(
            nn.Conv2d(1, degradation_hidden_dim, 3, stride=1, padding=1),
            nn.GELU(),
            nn.Conv2d(degradation_hidden_dim, degradation_hidden_dim, 3, stride=2, padding=1),
            nn.GELU(),
            nn.Conv2d(degradation_hidden_dim, degradation_hidden_dim, 3, stride=2, padding=1),
            nn.GELU(),
        ...
        self.degradation_projection = nn.Sequential(
            nn.Linear(degradation_hidden_dim + 4, degradation_dim),
            nn.GELU(),
            nn.Linear(degradation_dim, degradation_dim),
        )
        self.layer_conditioners = nn.ModuleList(
            [nn.Linear(degradation_dim, self.embed_dim * 4) for _ in range(self.num_layers)]
        )
        self.body_conditioner = nn.Linear(degradation_dim, self.embed_dim * 2)
        if hasattr(self, "conv_before_upsample"):
            num_feat = self.conv_before_upsample[0].out_channels
            self.upsample_conditioner = nn.Linear(degradation_dim, num_feat * 2)
        else:
            self.upsample_conditioner = None

    def _image_statistics(self, x):
        mean = x.mean(dim=(2, 3))
        std = x.std(dim=(2, 3), unbiased=False)
        grad_x = torch.mean(torch.abs(x[:, :, :, 1:] - x[:, :, :, :-1]), dim=(2, 3))
        grad_y = torch.mean(torch.abs(x[:, :, 1:, :] - x[:, :, :-1, :]), dim=(2, 3))
        return torch.cat([mean, std, grad_x, grad_y], dim=1)

    def encode_degradation(self, x):
        feat = self.degradation_encoder(x).flatten(1)
        stats = self._image_statistics(x)
        return self.degradation_projection(torch.cat([feat, stats], dim=1))

    def _apply_spatial_condition(self, x, affine):
    ...
    def forward_features(self, x, params, cond_embedding=None):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if cond_embedding is not None:
                layer_condition = torch.tanh(self.layer_conditioners[idx](cond_embedding))
                layer_params = dict(params)
                layer_params["layer_condition"] = layer_condition * self.degradation_condition_scale
            x = layer(x, x_size, layer_params)
    ...
    def forward(self, x):
        ...
        self.mean = self.mean.type_as(x)
        x = (x - self.mean) * self.img_range
        degradation_embedding = self.encode_degradation(x)
        ...
        if self.upsampler == "pixelshuffle":
            x = self.conv_first(x)
            body = self.conv_after_body(self.forward_features(x, params, degradation_embedding)) + x
            body = self._apply_spatial_condition(body, self.body_conditioner(degradation_embedding))
            x = self.conv_before_upsample(body)
            if self.upsample_conditioner is not None:
                x = self._apply_spatial_condition(x, self.upsample_conditioner(degradation_embedding))
            x = self.conv_last(self.upsample(x))
        elif self.upsampler == "pixelshuffledirect":
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, params, degradation_embedding)) + x
            x = self._apply_spatial_condition(x, self.body_conditioner(degradation_embedding))
            x = self.upsample(x)
        elif self.upsampler == "nearest+conv":
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, params, degradation_embedding)) + x
            ...
            x_first = self.conv_first(x)
            res = self.conv_after_body(self.forward_features(x_first, params, degradation_embedding)) + x_first
            res = self._apply_spatial_condition(res, self.body_conditioner(degradation_embedding))
            x = x + self.conv_last(res)
